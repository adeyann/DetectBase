...
- **GStreamer rtpmanager long-running leak (2026-05-19, accepted)**: External lib (`libgstrtpmanager.so`) leaks ~320 byte per EOS reconnect cycle. Call stack: `TeardownPipeline → set_state(NULL) → rtspsrc → rtpbin → rtpsession sources g_list_free_full → g_object_unref → rtpmanager internal finalize`. Our `TeardownPipeline` (GstRtspReceiver.cpp:178) is verified correct (state NULL transition + 5s wait + matched unref). ~340 MB/year long-term acceptable. **Do not attempt to "fix" this in our code** — call stack confirms internal GStreamer leak. v1.0.0 후 GStreamer 1.24+ upgrade 로 재시도 예정 (현 1.20.3, Ubuntu 22.04 jammy default).
- **GstRtspClient cam stale state (2026-05-20, root cause 추적 중)**: 8h 운영 후 일부 cam 의 RTP frame 멈춤. TCP ESTAB 유지 + EOS 안 받음 + `reset_cnt` freeze. 외부 RTSP server 정상 (별도 client 로 검증). 1회 발생 (cam 659). 자세한 timeline + 진단 절차 = [logs/STUCK_ANALYSIS_cam659_20260520.md](logs/STUCK_ANALYSIS_cam659_20260520.md). PR #16 진단 binary (`bus_message_total` / `reset_attempt_total` / `last_frame_age_sec` metric + 모든 bus message log) 적용 중. 사용자 정책상 자동 복구 (watchdog/forced restart) 금지 — root cause 식별 전 cover 안 함.
- **`correlation_mismatch` 폭증 (2026-05-20, PR #16+#17 binary trigger 추적 중)**: PR #16 진단 binary 적용 후 mismatch ~70× 폭증 (0.4 → 27.5/cam/sec). 모든 cam, 모든 mismatch 의 delta = 정확히 10 frame (cam 내부 NPU response result_q stable backlog). 동시 이상: prof_RSP ifq/resp 분포 반전, HWM 1GB peak, prof_INF push 16→41us. 운영 영향: frame 시차 ~330ms (bbox / tracking / event detection 정확도). q_drop 0, cam 4/4 OK. 가설: 진단 코드 의 atomic store + metric IncrementCounter 의 cache contention. 자세한 분석 + Phase 1~3 plan = [logs/MISMATCH_SURGE_ANALYSIS_20260520.md](logs/MISMATCH_SURGE_ANALYSIS_20260520.md).

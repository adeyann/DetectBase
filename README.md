...
./detectbase.sh audit      # 정적분석 + ASan/UBSan (운영 90초 정지)
...
| `engines/engine.classes.yaml` | 탐지 클래스 정의 (사람 / 차량 등) | 분기 시 변경 |
    ...
    "Engines": [
        {
            ...
            "ClassesPath":    "/DetectBase/engines/engine.classes.yaml",
            "Enable":         true
        }
    ]
}
...
- 목표: happytimesoft 외부 RTSP 라이브러리 → GStreamer (`rtspsrc + mppvideodec`) 로 교체. MPP NPU decoder 활용으로 CPU 부담 ↓.
...
- 시도 코드는 `.deleted_backup/gst_attempt_20260515/` 에 보존, 자료는 [logs/GSTREAMER_DEEP_REVIEW.md](logs/GSTREAMER_DEEP_REVIEW.md), [logs/ONVIF_PAYLOADER_DESIGN.md](logs/ONVIF_PAYLOADER_DESIGN.md) (재작업 시 참고)
...
| **engines/engine.classes.yaml** | 도메인별 클래스 정의 (사람/차량 외 추가) |
...
# 3. 운영 60분 누적 (RSS/FD/Threads leak 점검)
# logs/test_48h_*/snap_*.txt 추이 모니터링
...
| 문서 | 내용 |
|---|---|
| **[logs/NEXT_SESSION.md](logs/NEXT_SESSION.md)** | 현재 작업 계획서 (happytimesoft baseline 복원 → git push → GStreamer 재작업 branch) |
| [.DOCS/REVIEW3_COMPLETION_BASELINE_20260513.md](.DOCS/REVIEW3_COMPLETION_BASELINE_20260513.md) | 3차 코드리뷰 완료 시점 baseline 스냅샷 (구 SESSION_RESUME.md) |
...
| [.DOCS/REVIEW2/INDEX.md](.DOCS/REVIEW2/INDEX.md) | 2차 리뷰 진입점 (9개 흐름) — **레거시** (.DOCS/) |

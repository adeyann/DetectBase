...
**Version**: `0.1.28` (cmake `code/CMakeLists.txt`, develop 작업 진입 — master tag 보다 +10 patch). **last released master tag = `v0.1.18` (2026-05-27)**. v0.1.0 (5/19) → v0.1.18 (5/27) 누적 cam stuck fix + `.claude/settings.json` 권한 allow/deny 모델. **0.1.27 (5/28)** — **NPU batch_size 확장성 fix**: 이전 `YoloV5_Torch_Onnx_RKNN_NPU.cpp` 의 batch 처리 막힘 패턴 3건 정정 — (a) batch dim check 의미 정정 (`input_attrs[0].dims[0]` 비교, 이전 `io_num.n_input` 비교는 의미 부정확), (b) input buffer size 에 `* batch_size_` 곱하기, (c) Preprocess memcpy 에 `curr_batch_input_idx * frame_size` offset 적용 + 매 frame memset 제거 (이전 frame 보존). batch=1 운영 영향 0 (현 운영 default 유지), batch>1 진입 시 input 측 막힘 해소. **.rknn 모델 재변환 / post-processing batch 검증 / NPU throughput 실측은 batch>1 진입 시점 별도 작업 (OPERATIONS.md §10)**. branch naming rule 추가: branch 이름에 cmake VERSION 넣지 말 것 (`chore/v0.1.26-cycle` 같은 이름은 cycle 종료 후 stale, 새 규칙으로 `fix/npu-batch-size` 같은 scope-only 명만 허용). monitor alert escalation = 미적용 참고 옵션 (OPERATIONS §8). **0.1.19~0.1.26 (5/27~5/28) 누적 변경** — (1) **GMainContext UAF fix (0.1.26)**: `g_source_remove(guint id)` 가 default global context 만 검색하므로 per-instance `ctx_` 안 source 못 찾던 결함 → `g_source_destroy(GSource*)` + `g_source_unref` 패턴으로 변경, GMainContext per-instance 격리 의도 유지. ASan/TSan 1h 검증 통과 (TSan SEGV 0, baseline race profile 동일 회복). (2) **GstRtspReceiver/ProxyServer dedicated GMainContext (0.1.25)** — 모든 source (bus watch, jitter timer) 가 각 instance 자체 `ctx_` 에 attach. multi-cam coupling 해소. (3) **CLOSE-WAIT defensive close (0.1.24)** — TeardownPipeline 의 socket cleanup. (4) **SafeQueue MO-1 (0.1.23, 0.1.22)**: `notify_one()` / `notify_all()` 을 `lock_guard` 밖으로 이동, race deep review 결과 자체 race 0건 재확인 (`.DOCS/SAFEQUEUE_RACE_REVIEW_20260527.md`). (5) **DeviceCluster 인라인화 (0.1.20)** — `CameraCluster_DETECTOR` 를 `SettingManager` 에 흡수, 1-use 클래스 폐기. (6) **audit 강도 모드 (0.1.26)**: `./detectbase.sh audit` light default (ASan 60m + TSan 60m, ~1h 30min, develop/내부 검증) / `--strict` (ASan 240m + TSan 60m, ~5h, master merge gate). `ASAN_DURATION_MIN` / `TSAN_DURATION_SEC` env override 우선. (7) **git workflow rule 강화 (0.1.26)**: docs/small-commit branch PR/merge 금지 (PR #29 사고 후), branch cleanup hint pattern (local merge 후), remote state verification (`git ls-remote` / `fetch --prune` 우선, `git branch -a` 부정확). skill/memory 역할 분담 (procedural SSOT = skill, personal context = memory). **TeardownPipeline unref-skip on stuck (0.1.18)** — cam 661 의 42분 cam_loss 분석 결과 `gst_object_unref(pipeline_)` 가 GStreamer 내부 thread join 에서 unbounded block 하던 게 root cause. `gst_element_get_state` timeout 시 unref 건너뛰고 의도된 leak (process restart 시 OS cleanup) + WARN log. **5/26~5/27 11.3h 후속 모니터 wd=1 (boot only) / cam_loss=0** (pre-fix 50분 wd=6/cam_loss 영구). 단 fix path 미발화 (자연 stuck 안 일어남) → fix 실제 발휘 검증은 다음 stuck 시점에 가능. **git workflow 정책 갱신 + pre-push docs check 절대 규칙 + memory 영어화 (0.1.17)** — code/cmake bump 분리 (push 후 별도 commit), 머지 시 사용자 버전 확인, post-merge placeholder bump, 모든 commit push 시 docs 전수 점검. AI-only memory 디렉토리 영어 단일 언어화. **argv guard + flock single-instance lock (0.1.16)** — Main.cpp 의 `int main()` 가 argv 무수신 → `--version` 같은 일반 호출이 풀 서비스 spawn 하던 사고 (PID 4924, DFPS 50% 하락) 차단. argv 명시 case 외 FATAL exit 2 + `/DetectBase/logs/.detectbase.lock` 의 `flock(2)` advisory lock 으로 두 번째 instance 부팅 차단. monitor.sh 에 threshold alert 7 종 (storm/err/dfps_low/memory/wd/ftc/cam_loss) + warmup grace 4 cycle 추가. **REST response JSON parse silent catch 가시화 (0.1.15)** — `rest_impl.cpp` 의 catch 에 `MLOG_WARN` 추가. **MPP + Option A 완전 폐기 (0.1.14)** — mppvideodec 미사용 상태에서 Option A 의 14ms partial reset 이 multi-cam cluster sync 강화 → DFPS dip 증폭 확인. 5/24 Full reset baseline (mean DFPS 115.6, ≥110: 98.8%) 복귀. snapshot: tag `mpp-architecture-snapshot-v0.1.13` + `.backup/mpp_purged_20260526/`. **per-cam stage FPS counter (0.1.12) revert (0.1.13)** — global mutex hot path 으로 wd 빈도 증가 회귀, A-B-A control test 검증.

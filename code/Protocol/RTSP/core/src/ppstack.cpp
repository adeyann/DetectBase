...
#if 0
	if( ctx_ptr ) {
		MLOG_INFO( "[%s()] NUM(%lu) SIZE(%lu) bMtx(%d) ======> %p", __FUNCTION__, node_num, content_size, bNeedMutex, ctx_ptr );
	}
#endif
	return ctx_ptr;
}
...
BOOL pps_fl_push(PPSN_CTX * pps_ctx, void * content_ptr)
{
	...
	{
		log_print(H_LOG_WARN, "%s, unit ptr error!!!\r\n", __FUNCTION__);
		return FALSE;
	}
	...
	{
		log_print(H_LOG_WARN, "%s, unit node %d already in freelist !!!\r\n", __FUNCTION__, pps_get_index(pps_ctx, content_ptr));
	...
	//MLOG_INFO( "[%s()] NODE(%lu) POP(%lu) ======> %p", __FUNCTION__, pps_ctx->node_num, pps_ctx->pop_cnt, pps_ctx );
	return TRUE;
}
...
PPSN * pps_ctx_ul_del_node_unlock(PPSN_CTX * ul_ctx, PPSN * p_node)
{
	...
	{
		log_print(H_LOG_WARN, "%s, unit not in used list!!!\r\n", __FUNCTION__);
		return NULL;
	}

	if (ul_ctx->head_node == 0)
	{
		log_print(H_LOG_WARN, "%s, used list is empty!!!\r\n", __FUNCTION__);
		return NULL;
	}
...
uint32 pps_get_index(PPSN_CTX * pps_ctx, void * content_ptr)
{
	...
	if (offset != 0)
	{
		...
		log_print(H_LOG_WARN, "%s, unit ptr error,pps_ctx[0x%08x],ptr[0x%08x],low_offset[0x%08x],offset[0x%08x],like entry[%u]\r\n",
...
void * pps_get_node_by_index(PPSN_CTX * pps_ctx, unsigned long index)
{
	...
	if (content_offset > pps_ctx->high_offset)
	{
		if (index != 0xFFFFFFFF)
		{
			log_print(H_LOG_WARN, "%s, index [%u]error!!!\r\n", __FUNCTION__, index);
		}
		return NULL;
	}
...
void pps_wait_mutex(PPSN_CTX * pps_ctx)
{
	if (pps_ctx == NULL)
	{
		log_print(H_LOG_WARN, "%s, pps_ctx == NULL!!!\r\n", __FUNCTION__);
		return;
	}

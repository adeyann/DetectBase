...
hqBufPut_start:
	...
	if (queue_count >= (phq->unit_num - 1))
	{
		if ((phq->queue_mode & HQ_NO_EVENT) == 0)
		{
			...
			{
				if (phq->count_put_full < HQ_MAX_WAIT_COUNT) {
					phq->count_put_full++;
					int ret = sys_os_sig_wait_timeout(phq->queue_nfulEvent, 10);
					if (ret == -1) {
						hqPutMutexLeave(phq);
						return FALSE;
					}
...
BOOL hqBufPeekRear(HQUEUE* phq, char* buf)
{
	uint32 real_rear;
...
hqBufPeekRear_start:
	...
	real_rear = (phq->rear - 1) % phq->unit_num;
	memcpy(buf, ((char*)phq) + phq->queue_buffer + real_rear * phq->unit_size, phq->unit_size);

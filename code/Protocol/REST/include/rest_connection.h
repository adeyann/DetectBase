...
class Connection {
 public:
    ...
    // 새로 추가: 예외를 던지는 함수
    Response GET_or_throw_if_timeout( const std::string& url, int timeoutSeconds = 3 );

    // wait_until, wait_for 개념을 이용한 오버로딩
    Response GET_or_throw_if_timeout( const std::string& url, std::chrono::time_point<std::chrono::steady_clock> timeoutTime );
    Response GET_or_throw_if_timeout( const std::string& url, std::chrono::seconds timeoutDuration );

    // check reachable conn
    bool is_rest_api_alive( void );
    ...
    // mutable std::mutex rest_clinet_connect_mtx;
};
};  // namespace RestClient

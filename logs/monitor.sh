...
while true; do
    ...
    # § SAMPLE — DFPS (metric 기반) / resident / jem (log 기반)
    # 이전: log 의 'TotalDFPS:' grep — DEBUG_MODE compile-out (v0.1.20+) 후 emit 안 되어 DFPS=0 결함
    # (v0.1.27 41h monitor + v0.1.28 30min sanity 모두 dfps=0 표기 확인).
    # 변경: metric endpoint 직접 curl. metric `detectbase_dfps_total` 은 Release/Debug 모두 always-on.
    DFPS=$(curl -s --max-time 3 "$METRICS_URL" 2>/dev/null | awk '/^detectbase_dfps_total / {printf "%.1f", $NF; exit}')

...
- **Git workflow** — AI may use git/gh but **never commits directly to `master` or `develop`**. All work happens on dedicated branches (feature/fix/chore/...) forked from develop; both develop merge and master merge go via PR (`gh pr create` + `gh pr merge`).
  ...
  - **Branch naming — version 금지 (2026-05-28 신규)** — branch 이름에 cmake VERSION 또는 cycle 버전을 넣지 마라. branch 는 work scope 표현용이고 version 은 `code/CMakeLists.txt` / git tag / commit 의 영역이다. 위반 예: `chore/v0.1.26-cycle`, `feature/0.2.0-multi-engine`. 권장: `chore/cycle-cleanup`, `feature/multi-engine`, `fix/npu-batch-size`. 이유: cycle 종료 / 머지 / 폐기 시점에 branch 이름이 stale 정보가 됨. work scope (어떤 변경인지) 만 표기.

...
- **Single-line Bash by default (avoid permission-prompt flood).** Claude Code's allow patterns match commands as a whole; multi-line compounds (commands containing newlines) often miss the pattern and trigger a prompt each time. Discipline: chain with `&&` / `;` / `|` on one line. For logic that would exceed ~5 lines, write a helper to `scripts/*.py` or `scripts/*.sh` (already allowed) and invoke it as a single-line `./scripts/X.py` call. Reserve heredocs/multi-line bash for genuinely unavoidable one-offs.

...
./detectbase.sh audit      # 정적분석 + ASan/UBSan (cppcheck + clang-tidy + ASan) — 운영 90초 정지
...
3. 빈발 시 `./detectbase.sh audit --with-tsan` 으로 race / deadlock 검사
...
```bash
./detectbase.sh audit              # 4종 도구 (cppcheck + clang-tidy + ASan + UBSan), 운영 90초 정지
./detectbase.sh audit --with-tsan  # 위 + TSan, 운영 30초 추가 정지
```
...
**기준 (DetectBase production-ready baseline)**:
- cppcheck 자체 코드 결함: 50건 (모두 style — 외부 알고리즘)
- clang-tidy warning: 90건 (대부분 false positive)
- ASan/UBSan: 외부 librknnrt.so leak 만 (자체 코드 0건)
- TSan: 187 warning 중 cv_any false positive 18건 + 외부 RTSP — 자체 코드 진짜 race 0건
...
- [ ] `./detectbase.sh audit --with-tsan` 실행 → 결과 비교 (`logs/audit_<timestamp>/summary.txt`)

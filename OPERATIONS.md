...
- ASan/UBSan: startup leak 0 ✅, runtime leak 1 (GStreamer rtpmanager 외부 lib, 수용. ~340 MB/year — 5/19 1h baseline 기준. 현재 default 4h run 으로 갱신 예정)
- TSan: **자체 코드 진짜 race 0 ✅** (4 root cause 모두 fix). 잔여 ~137 WARNING (SIGKILL false positive + 외부 lib + 추적 한계 — 5/19 5분 run 기준. 현재 default 1h run 으로 갱신 예정)

...
./detectbase.sh audit --only asan|ubsan  # 동적 (운영 정지)
./detectbase.sh audit --only tsan        # 동적 (운영 정지, 5분)
```
...
**기준 (DetectBase production-ready baseline — 2026-05-19 v0.1.0)**:
- cppcheck 자체 코드 결함: **63건** (false positive 20 + Profiler 자연정리 9 + needs-review 11)
- clang-tidy warning: **30건** (needs-review 24 + 누락 narrowing 4 + exception-escape FP 2)
- ASan/UBSan: startup leak 0 ✅, runtime leak 1 (GStreamer rtpmanager 외부 lib, 수용)
- TSan: **자체 코드 진짜 race 0 ✅** (UAF / init race / map race / shared_ptr atomic 4 root cause 모두 fix). 잔여 139 = SIGKILL false positive + GStreamer 내부 + SafeQueue 추적 한계 (운영 영향 0)

자세한 내역: [logs/AUDIT_REPORT_20260519.md](logs/AUDIT_REPORT_20260519.md), [logs/SESSION_DFPS_B3_B4_PLATEAU_20260519.md](logs/SESSION_DFPS_B3_B4_PLATEAU_20260519.md).
